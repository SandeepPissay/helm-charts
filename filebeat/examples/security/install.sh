...
sed "s/%ELASTICSEARCH_IPADDRESS%/$ES_IPADDRESS/g" values.yaml > /tmp/${TKC_NAME}-values.yaml && \
	helm -n $TKC_NAMESPACE upgrade --wait --debug --timeout=1200s --install --values /tmp/${TKC_NAME}-values.yaml helm-filebeat-security ../../
